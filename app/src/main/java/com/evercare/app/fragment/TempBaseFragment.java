...
public class TempBaseFragment extends BaseFragment {
    ...
    protected void initView(View view) {
        ...
        mLinearLayoutManager = (LinearLayoutManager) customerRecyclerView.getLayoutManager();
        customerRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (move) {
                    ...
                    int num2 = customerRecyclerView.getChildCount();
                    if (0 <= n && n < adapter.getItemCount()) {
                        //获取要置顶的项顶部离RecyclerView顶部的距离
                        int top = customerRecyclerView.getChildAt(n).getTop();
                        //最后的移动
                        customerRecyclerView.scrollBy(0, top);
                    }*/
                }
            }
        });
        ...
        sidebar_Letters.setOnTouchingLetterChangedListener(new SideBar.OnTouchingLetterChangedListener() {
            ...
            public void onTouchingLetterChanged(String s) {
                if (adapter != null) {
                    mIndex = adapter.getPositionForSelection(s.charAt(0));
                    if (mIndex >= 0) {
                        //customerRecyclerView.scrollToPosition(mIndex);
    ...
    private void moveToPosition(int n) {
        ...
        int firstItem = ((LinearLayoutManager) customerRecyclerView.getLayoutManager()).findFirstVisibleItemPosition();
        int lastItem = ((LinearLayoutManager) customerRecyclerView.getLayoutManager()).findLastVisibleItemPosition();
        //然后区分情况
        if (n <= firstItem) {
            //当要置顶的项在当前显示的第一个项的前面时
            customerRecyclerView.scrollToPosition(n);
        } else if (n <= lastItem) {
            //当要置顶的项已经在屏幕上显示时
            int top = customerRecyclerView.getChildAt(n - firstItem).getTop();
            customerRecyclerView.scrollBy(0, top);
        } else {
    ...
    protected void initData() {
        ...
        try {
            jsonObject.put("type", Constant.TEMPORARY);
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    if (!TextUtils.isEmpty(response.getString("data"))) {
                        ...
                        if (TextUtils.equals(code, Result.SUCCESS) && personList != null) {
