 ...
 * 活动信息类Adapter
 */
public class CaseListAdapter extends RecyclerView.Adapter<CaseListAdapter.HeaderViewHolder> {
    private Context context;
    private LayoutInflater layoutInflater;
    private List<ActivityItemInfo> caseItemInfos;
    ...
    public CaseListAdapter(Context context, List<ActivityItemInfo> list, CustomerItemClickListener itemClickListener) {
...
//        if (position > 0 ) {
            if (holder instanceof ViewHolder) {
                ActivityItemInfo itemInfo = caseItemInfos.get(position - 1);
                if (holder instanceof ViewHolder) {
                    Uri uri = Uri.parse(itemInfo.getImage());
                    ((ViewHolder) holder).simpleimage.setImageURI(Constant.BASEURL_IMG +uri);
                    ((ViewHolder) holder).txtContent.setText(itemInfo.getTitle());
                }
...
//            ActivityItemInfo itemInfo = caseItemInfos.get(position - 1);
//            if (holder instanceof ViewHolder) {
//                Uri uri = Uri.parse(Constant.BASEURL + itemInfo.getImage());
//                ((ViewHolder) holder).simpleimage.setImageURI(uri);
//                ((ViewHolder) holder).txtContent.setText(itemInfo.getTitle());
//            }
//        }
    }
    ...
    public class FooterViewHolder extends HeaderViewHolder {
